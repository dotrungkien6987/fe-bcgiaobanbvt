...
function DanhSachGiaoViec({ onOpenDialog }) {
  ...
  const filteredData = useMemo(() => {
    const giaoViecData = giaoViecs.filter(
      (gv) => gv.LoaiQuanLy === "Giao_Viec"
    );

    if (!searchText) return giaoViecData;

    return giaoViecData.filter((item) => {
                        ...
                        <Stack spacing={2} alignItems="center">
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {isLoading
                              ...
                              : "Chưa có nhân viên nào được giao việc"}
                          </Typography>
                        </Stack>
                      </TableCell>
                    </TableRow>
