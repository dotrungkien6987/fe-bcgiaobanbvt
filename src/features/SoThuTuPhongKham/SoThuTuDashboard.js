...
const SoThuTuDashboard = () => {
    ...
    if (!isLoading) {
      ...
      if (
        ...
        setTimeout(() => {
          ...
          if (phongThucHienCardRef.current)
            phongThucHienCardRef.current.resetScroll();
          if (phongLayMauCardRef.current)
            phongLayMauCardRef.current.resetScroll();
  ...
  const fetchData = (resetData = true) => {
    ...
    if (
      roomTypes.phongKham &&
      roomTypes.phongThucHien &&
      roomTypes.phongLayMau
    ) {
  ...
  const displayData = {
    phongKham:
      isLoading && cachedData.phongKham.length > 0
        ? cachedData.phongKham
        : phongKhamData,
    phongThucHien:
      isLoading && cachedData.phongThucHien.length > 0
        ? cachedData.phongThucHien
        : phongThucHienData,
    phongLayMau:
      isLoading && cachedData.phongLayMau.length > 0
        ? cachedData.phongLayMau
        : phongLayMauData,
  };
  ...
  return (
    ...
    >
                ...
                >
                  ...
                  >
                    <Chip
                      ...
                      icon={
                        <AutorenewIcon
                          ...
                          sx={{
                            ...
                            animation: isAutoRefreshEnabled
                      ...
                      label={
                        isAutoRefreshEnabled ? `${secondsUntilRefresh}s` : "Tắt"
                      }
              ...
              {lastUpdatedTime && (
                ...
                >
                  <Tooltip
                    title={
                      isAutoRefreshEnabled
                        ? `Tự động cập nhật mỗi phút | Cập nhật lúc: ${formatTime(lastUpdatedTime)}`
                        : `Đã tắt tự động cập nhật | Cập nhật lúc: ${formatTime(lastUpdatedTime)}`
                    }
                  >
                    <Chip
                      size="small"
      ...
      >
        ...
        {isLoading &&
          cachedData.phongKham.length +
            cachedData.phongThucHien.length +
            cachedData.phongLayMau.length >
            0 && (
            <Box
              sx={{
                position: "absolute",
