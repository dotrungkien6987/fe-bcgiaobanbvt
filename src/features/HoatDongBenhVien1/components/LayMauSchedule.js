...
import { Grid, Paper, Typography, Box, Divider, Chip, CircularProgress, Tooltip, IconButton } from '@mui/material';
import { useHoatDongBenhVien } from '../HoatDongBenhVienProvider';
import { DEPARTMENT_TYPES } from '../constants';
import { useSelector, useDispatch } from 'react-redux';
import { selectSoThuTuPhongLayMau, getAllSoThuTuStats } from '../../Slice/soThuTuSlice';
...
const LayMauSchedule = ({ isCompactView }) => {
  ...
  const { getDepartmentsByType, schedules, visibleTypes, loadingSoThuTu, selectedDate } = useHoatDongBenhVien();
  ...
  const refreshDepartmentData = (departmentId) => {
    ...
    const departmentIds = currentKhoa && currentKhoa.HisDepartmentID ? [currentKhoa.HisDepartmentID] : [];
    ...
    return (
      <Grid item xs={12} md={isCompactView ? 6 : 12} lg={isCompactView ? 4 : 12} key={department.id}>
        ...
        >
          ...
          {phongLayMauData && phongLayMauData.length > 0 && (() => {
            ...
            const soThuTuData = currentKhoa && currentKhoa.HisDepartmentID ? 
              phongLayMauData.find(item => 
                item.departmentid && item.departmentid.toString() === currentKhoa.HisDepartmentID.toString()
              ) : null;
            
            if (soThuTuData) {
              const lastUpdated = soThuTuData.lastupdated ? new Date(soThuTuData.lastupdated) : null;
              ...
              return (
                <>
                  ...
                  <Box sx={{ 
                    ...
                    <Chip 
                      label={`STT: ${soThuTuData.latest_sothutunumber_da_lay_mau || 'N/A'}`} 
                      color="primary" 
                      size="small" 
                      variant="outlined"
                      sx={{ fontWeight: 'bold' }}
                    ...
                    <Grid container spacing={1} sx={{ mb: 1.5 }}>
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary">Tổng mẫu cần lấy</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <ScienceIcon fontSize="small" sx={{ mr: 0.5, color: color }} />
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary">Tổng bệnh nhân</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <PeopleAltIcon fontSize="small" sx={{ mr: 0.5, color: color }} />
                            <Typography variant="h6" fontWeight="bold" sx={{ color: color }}>
                    ...
                    <Grid container spacing={1} sx={{ mb: 1.5 }}>
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>Chưa lấy mẫu</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>Đã lấy mẫu</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>BN chưa lấy mẫu</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <PersonIcon fontSize="small" color="warning" sx={{ mr: 0.5 }} />
                            <Typography variant="h6" fontWeight="bold" color="warning.main">
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>BN đã lấy mẫu</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Grid container spacing={1} sx={{ mb: 1.5 }}>
                      <Grid item xs={4}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>Chưa thực hiện</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <HourglassEmptyIcon fontSize="small" color="warning" sx={{ mr: 0.5 }} />
                            <Typography variant="h6" fontWeight="bold" color="warning.main">
                      ...
                      <Grid item xs={4}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>Chờ kết quả</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <ArrowForwardIcon fontSize="small" color="info" sx={{ mr: 0.5 }} />
                            <Typography variant="h6" fontWeight="bold" color="info.main">
                              {soThuTuData.so_ca_da_thuc_hien_cho_ket_qua || 0}
                            </Typography>
                          </Box>
                        </Box>
                      </Grid>
                      ...
                      <Grid item xs={4}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary" noWrap>Đã trả kết quả</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <CheckCircleIcon fontSize="small" color="success" sx={{ mr: 0.5 }} />
                            <Typography variant="h6" fontWeight="bold" color="success.main">
                        ...
                        <Grid item xs={6}>
                          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <Typography variant="body1" fontWeight="bold" color="info.main">
                              {soThuTuData.max_sothutunumber_da_lay_mau || 'N/A'}
                            </Typography>
                          </Box>
                        </Grid>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', mb: 1 }}>
                    <Chip
                      icon={<AccessTimeIcon />}
                      label={`Lấy mẫu tiếp theo: ${soThuTuData.sothutunumber_du_kien_tiep_theo || 'N/A'}`} 
                      size="medium"
                      variant={soThuTuData.sothutunumber_du_kien_tiep_theo ? "default" : "outlined"}
                      sx={{ 
                        fontWeight: 'bold',
                        py: 1,
                        fontSize: '1rem',
