...
import { selectSoThuTuPhongKham, getAllSoThuTuStats } from '../../Slice/soThuTuSlice';
...
const NgoaiTruSchedule = ({ isCompactView }) => {
  const dispatch = useDispatch();  const { getDepartmentsByType, schedules, loadingSoThuTu, selectedDate } = useHoatDongBenhVien();
    ...
    return (
      <Grid item xs={12}>
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="body1">Không có dữ liệu khoa ngoại trú</Typography>
        </Paper>
      </Grid>
    );
      ...
      <Grid item xs={12} md={isCompactView ? 6 : 12} lg={isCompactView ? 4 : 12} key={department.id}>
        ...
        >          {(loadingSoThuTu || refreshing[department.id]) && (
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            ...
            {/* Thông tin số thứ tự */}
          {phongKhamData && phongKhamData.length > 0 && (() => {            // Tìm khoa hiện tại trong danh sách khoa từ redux
                      ...
                      {soThuTuData.lastupdated && (
                        <Tooltip title="Cập nhật lần cuối" placement="top">
                          <Typography variant="caption" color="text.secondary" sx={{ ml: 1 }}>
                            ({new Date(soThuTuData.lastupdated).getHours().toString().padStart(2, '0')}:
                            {new Date(soThuTuData.lastupdated).getMinutes().toString().padStart(2, '0')})
                          </Typography>
                        </Tooltip>
                      )}
                    </Box>
                    <Chip 
                      label={`STT hiện tại: ${soThuTuData.latest_benh_nhan_da_kham || 'N/A'}`} 
                      color="primary" 
                      size="small" 
                      variant="outlined"
                      sx={{ fontWeight: 'bold' }}
                    ...
                    <Grid container spacing={1} sx={{ mb: 1.5 }}>
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <PeopleAltIcon fontSize="small" color="primary" sx={{ mr: 0.5 }} />
                            <Typography variant="h6" fontWeight="bold">{soThuTuData.tong_benh_nhan || 0}</Typography>
                          </Box>
                        </Box>
                      </Grid>
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary">Chưa khám</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary">Đang khám</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Grid item xs={6}>
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" color="text.secondary">Đã khám xong</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
