...
export const selectCurrentNhomKhoa = (state) =>
  state.nhomKhoaSoThuTu.currentNhomKhoa;
export const selectDepartmentIds = (state) =>
  state.nhomKhoaSoThuTu.departmentIds;
...
export const getLookupNhomKhoa = () => async (dispatch) => {
  dispatch(slice.actions.startLoading());
  try {
    const response = await apiService.get("/nhomkhoasothutu/lookup");
    dispatch(slice.actions.getAllNhomKhoaSuccess(response.data.data));
  } catch (error) {
    dispatch(slice.actions.hasError(error.message));
    toast.error(error.message);
