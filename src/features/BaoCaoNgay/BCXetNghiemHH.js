...
const RegisterSchema = Yup.object().shape({
  ...
  HuyetHoc: Yup.number().typeError("Bạn phải nhập 1 số"),
 
});

function BCXetNghiemHH() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    setValue(
      "HuyetHoc",
      ctChiSos.find((obj) => obj.ChiSoCode === "xn-HuyetHoc")?.SoLuong || 0
    );
  ...
  const handleCapNhatDuLieu = (data) => {
    ...
    const ctChiSo = [
      ...
      { ChiSoCode: "xn-HuyetHoc", SoLuong: data.HuyetHoc },
      ...
      <Stack>
        ...
        >
          ...
          <Grid container spacing={3} my={1}>
            ...
            <Grid item xs={6} md={4}>
              <FTextField name="HuyetHoc" label="Tổng số XN huyết học" />
            </Grid>
