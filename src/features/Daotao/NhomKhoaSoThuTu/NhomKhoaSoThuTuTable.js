...
function NhomKhoaSoThuTuTable() {
  ...
  const { user } = useAuth();
  const canManageNhomKhoa = ["admin", "superadmin", "daotao"].includes(
  ...
  const { nhomKhoaList, isLoading } = useSelector(
    (state) => state.nhomKhoaSoThuTu,
  );
      ...
      <Grid item xs={12} lg={12}>
        <MainCard title="Danh mục nhóm khoa số thứ tự">
          <SimpleTable
            ...
            isLoading={isLoading}
            additionalComponent={
              canManageNhomKhoa ? <AddNhomKhoaButton /> : null
            }
          />
        </MainCard>
      </Grid>
